<?php
	...
	class crc_dean extends crc_object {
		...
		function fn_getschedulelist($db) {
			...
			if ($this->_DEBUG) {
				echo "DEBUG {crc_admin::fn_getcourselist}: Getting schedule list <br>";
			}
			...
			if ($db->m_mysqlhandle != 0) {
				$this->m_sql = 'select * ' .
								'from  ' . MYSQL_COURSES_TBL . ' as c, ' . MYSQL_SCHEDULE_TBL . ' as s '.
								'where (s.schedule_course_id = c.course_id and course_active = "1") ' .
				...
				if (mysql_num_rows($resource) > 0) {
					...
					$this->m_schedulelist = '';					
					while ($row = mysql_fetch_array($resource)) {
							$this->m_schedulelist[$index]['cnum'] = $row[1];
							$this->m_schedulelist[$index]['cnumname'] = $row[1] . ', ' . $row[2];
							$this->m_schedulelist[$index]['courseid'] = $row[0];
							$this->m_schedulelist[$index]['scheduleid'] = $row[5];
						$index++;
					}
				} else {
					$this->m_schedulelist = null;
					if ($this->_DEBUG) {
						echo 'ERROR {crc_admin::fn_getcourselist}: The sql command returned nothing. <br>';
					}
				}
...
*/
				$this->m_sql = 'select profile_id, profile_name from '. MYSQL_PROFILES_TBL .
							   ' where (profile_active = "1") and (profile_role_id between 1 and 2) ' .
...
*/
		...
		function fn_setstudentschedule($db, $post, $profileid) {
			...
			//delete existing schedules for inserting new ones
